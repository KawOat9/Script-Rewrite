/**
 * Automatic Policy Surge & Loon，根据当前网络自动切换策略组，主要用于搭配软路由等使用。
...
manager()
  .catch((err) => {
    notify.post("🤖️ SSID Automatic Policy", `❌ เกิดข้อผิดพลาด`, err);
...
async function manager() {
  ...
  if (isSurge) {
    ...
    if (!config.silence && !v4_ip) {
      notify.post("🤖️ SSID Automatic Policy", "❌ ไม่มีเครือข่ายในขณะนี้", "");
      return;
    }
  }
  ...
  if (!config.silence) {
    notify(
      "🤖️ SSID Automatic Policy",
      `เครือข่ายปัจจุบัน：${ssid ? ssid : "ข้อมูลเซลลูลาร์"}`,
      `${isSurge ? "Surge" : "Loon"}เปลี่ยนไปใช้${lookupOutbound(targetMode)}`
    );
  }
}
